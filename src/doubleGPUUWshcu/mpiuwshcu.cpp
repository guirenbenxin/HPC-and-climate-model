...
__device__ double trten_s[mix*mkx*ncnst];
//__device__ double trflx_s[mix*(mkx+1)*ncnst];


//!----- Variables for temporary storages

__device__ double qv0_o[mix*mkx], ql0_o[mix*mkx], qi0_o[mix*mkx], t0_o[mix*mkx], s0_o[mix*mkx], u0_o[mix*mkx], v0_o[mix*mkx];
__device__ double qt0_o[mix*mkx]    , thl0_o[mix*mkx]   , thvl0_o[mix*mkx]   ,                         
...
thv0bot_o[mix*mkx], thv0top_o[mix*mkx], thvl0bot_o[mix*mkx], thvl0top_o[mix*mkx],             
ssthl0_o[mix*mkx] , ssqt0_o[mix*mkx]  , ssu0_o[mix*mkx]    , ssv0_o[mix*mkx];     //qc_o[mix*mkx]    ,   
...
__device__ double tr0_o[mix*mkx*ncnst];
__device__ double sstr0_o[mix*mkx*ncnst];  //trten_o[mix*mkx*ncnst], 
//double trflx_o[(mkx+1)*ncnst];

__device__ double sstr0[mix*mkx*ncnst];//                              !  Linear slope of environmental tracers [ #/Pa, kg/kg/Pa ]


__device__ double trten[mix*mkx*ncnst];//                              !  Tendency of tracers [ #/s, kg/kg/s ]
__device__ double trflx[mix*(mkx+1)*ncnst];//                          !  Flux of tracers due to convection [ # * kg/m2/s, kg/kg * kg/m2/s ]

__device__ double tru[mix*(mkx+1)*ncnst];//                              !  Updraft tracers [ #, kg/kg ]

__device__ double tru_emf[mix*(mkx+1)*ncnst];//                          !  Penetrative Downdraft tracers at entraining interfaces [ #, kg/kg ] 

	//!----- Variables for implicit CIN computation
__device__ double qv0_s[mix*mkx]  , ql0_s[mix*mkx]   , qi0_s[mix*mkx]   , s0_s[mix*mkx]    , u0_s[mix*mkx]    ,      
           v0_s[mix*mkx]   , t0_s[mix*mkx]    , qt0_s[mix*mkx]    , qvten_s[mix*mkx] , //, thl0_s[mkx]  , thvl0_s[mkx]
           qlten_s[mix*mkx], qiten_s[mix*mkx] , qrten_s[mix*mkx] , qsten_s[mix*mkx] , sten_s[mix*mkx]  , evapc_s[mix*mkx] , 
           uten_s[mix*mkx] , vten_s[mix*mkx]  , cufrc_s[mix*mkx] , qcu_s[mix*mkx]   , qlu_s[mix*mkx]   , qiu_s[mix*mkx]   , 
           qc_s[mix*mkx]; ///    , qtten_s[mix*mkx] , slten_s[mix*mkx]; fer_s[mix*mkx]  , fdr_s[mix*mkx]   ,

__device__ double tr0[mix*mkx*ncnst];//[mkx*ncnst];//                                !  Environmental tracers [ #, kg/kg ]

__device__ double umf_s[mix*(mkx+1)], slflx_s[mix*(mkx+1)] , qtflx_s[mix*(mkx+1)];/// , ufrc_s[mkx+1]  , uflx_s[mkx+1] , vflx_s[mkx+1];

__device__ double    qlten_sink[mix*mkx];//                               !  Liquid condensate tendency by compensating subsidence/upwelling [ kg/kg/s ]
__device__ double    qiten_sink[mix*mkx];//                               !  Ice    condensate tendency by compensating subsidence/upwelling [ kg/kg/s ]
__device__ double    nlten_sink[mix*mkx];//                               !  Liquid droplets # tendency by compensating subsidence/upwelling [ kg/kg/s ]
__device__ double    niten_sink[mix*mkx];//                               !  Ice    droplets # tendency by compensating subsidence/upwelling [ kg/kg/s ]

__device__ double    evapc[mix*mkx];//                                    !  Tendency of evaporation of precipitation [ kg/kg/s ]
__device__ double    slflx[mix*(mkx+1)];//                                  !  Updraft/pen.entrainment liquid static energy flux [ J/kg * kg/m2/s ]
__device__ double    qtflx[mix*(mkx+1)];//                                  !  Updraft/pen.entrainment total water flux [ kg/kg * kg/m2/s ]
__device__ double    uflx[mix*(mkx+1)];//                                   !  Updraft/pen.entrainment flux of zonal momentum [ m/s/m2/s ]
__device__ double    vflx[mix*(mkx+1)];//                                   !  Updraft/pen.entrainment flux of meridional momentum [ m/s/m2/s ]
__device__ double    cufrc[mix*mkx];//                                    !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]

__device__ double    trflx_d[mix*(mkx+1)];//                                !  Adjustive downward flux of tracers to prevent negative tracers
__device__ double    trflx_u[mix*(mkx+1)];//                                !  Adjustive upward   flux of tracers to prevent negative tracers

__device__ double    thlu_emf[mix*(mkx+1)];//                               !  Penetrative downdraft liquid potential temperature at entraining interfaces [ K ]
__device__ double    qtu_emf[mix*(mkx+1)];//                                !  Penetrative downdraft total water at entraining interfaces [ kg/kg ]
__device__ double    uu_emf[mix*(mkx+1)];//                                 !  Penetrative downdraft zonal wind at entraining interfaces [ m/s ]
__device__ double    vu_emf[mix*(mkx+1)];//                                 !  Penetrative downdraft meridional wind at entraining interfaces [ m/s ]

__device__ double    uemf[mix*(mkx+1)];//                                   !  Net updraft mass flux at the interface ( emf + umf ) [ kg/m2/s ]
__device__ double    comsub[mix*mkx];//                                   !  Compensating subsidence at the layer mid-point ( unit of mass flux, umf ) [ kg/m2/s ]

__device__ double    qc_l[mix*mkx];//                                     !  Tendency due to detrained 'cloud water' (without rain-snow contribution) [ kg/kg/s ]
__device__ double    qc_i[mix*mkx];//                                     !  Tendency due to detrained 'cloud ice' (without rain-snow contribution) [ kg/kg/s ]

__device__ double    qrten[mix*mkx];//                                    !  Tendency of rain water specific humidity [ kg/kg/s ]
__device__ double    qsten[mix*mkx];//                                    !  Tendency of snow specific humidity [ kg/kg/s ]

__device__ double    qv0_star[mix*mkx];//                                 !  Environmental water vapor specific humidity [ kg/kg ]
__device__ double    ql0_star[mix*mkx];//                                 !  Environmental liquid water specific humidity [ kg/kg ]
__device__ double    qi0_star[mix*mkx];//                                 !  Environmental ice specific humidity [ kg/kg ]
///__device__ double    t0_star[mkx];//                                  !  Environmental temperature [ K ]
__device__ double    s0_star[mix*mkx];//                                  !  Environmental dry static energy [ J/kg ]

__device__ double    cldfrct[mix*mkx];//[mkx];//                                  !  Total cloud fraction at the previous time step [ fraction ]
__device__ double    concldfrct[mix*mkx];//[mkx];//                               !  Total convective cloud fraction at the previous time step [ fraction ]

__device__ double    flxrain[mix*(mkx+1)];//                                !  Downward rain flux at each interface [ kg/m2/s ]
__device__ double    flxsnow[mix*(mkx+1)];//                                !  Downward snow flux at each interface [ kg/m2/s ]
__device__ double    ntraprd[mix*mkx];//                                  !  Net production ( production - evaporation +  melting ) rate of rain in each layer [ kg/kg/s ]
__device__ double    ntsnprd[mix*mkx];//                                  !  Net production ( production - evaporation + freezing ) rate of snow in each layer [ kg/kg/s ]

__device__ double    thv0bot[mix*mkx];//                                  !  Environmental virtual potential temperature at the bottom of each layer [ K ]
__device__ double    thv0top[mix*mkx];//                                  !  Environmental virtual potential temperature at the top of each layer [ K ]
__device__ double    thvl0bot[mix*mkx];//                                 !  Environmental liquid virtual potential temperature at the bottom of each layer [ K ]
__device__ double    thvl0top[mix*mkx];//                                 !  Environmental liquid virtual potential temperature at the top of each layer [ K ]

__device__ double    dwten[mix*mkx];//                                    !  Detrained water tendency from cumulus updraft [ kg/kg/s ]
__device__ double    diten[mix*mkx];//                                    !  Detrained ice   tendency from cumulus updraft [ kg/kg/s ]

__device__ double    thvu[mix*(mkx+1)];//                                   !  Updraft virtual potential temperature at the interface [ m/s ]
__device__ double    rei[mix*mkx];//                                      !  Updraft fractional mixing rate with the environment [ 1/Pa ]

__device__ double    qvten[mix*mkx];//                                    !  Tendency of water vapor specific humidity [ kg/kg/s ]
__device__ double    qlten[mix*mkx];//                                    !  Tendency of liquid water specific humidity [ kg/kg/s ]
__device__ double    qiten[mix*mkx];//                                    !  Tendency of ice specific humidity [ kg/kg/s ]
__device__ double    sten[mix*mkx];//                                     !  Tendency of dry static energy [ J/kg ]
__device__ double    uten[mix*mkx];//                                     !  Tendency of zonal wind [ m/s2 ]
__device__ double    vten[mix*mkx];//                                     !  Tendency of meridional wind [ m/s2 ]
...
// 			}
		...
		//	if(i==220) printf("rho0inv=%f,sigmaw=%f,mu=%f\n",rho0inv,sigmaw,mu);
			...
			mulclstar = sqrt(fabs(max(0.0,2.0*pow((exp((-1)*pow(mu,2))/2.5066),2)*(1.0/pow(1.1,2)-0.25/pow(rmaxfrac,2)))));//!!!erfc(mu)__error_function_MOD_erfc(mu)
...
//		printf("i=%d,mulcl=%e,rmaxfrac=%e,mu=%e\n",i,mulcl,rmaxfrac,mu);
			...
			winv = sigmaw*(2.0/2.5066)*exp((-1)*pow(mu,2))/1.1;//__error_function_MOD_erfc(mu); //!!!erfc(mu)
...
//if(i == 7) printf("trflx[7,1,16]=%16.14f\n",trflx[16*(mkx+1)*iend+1*iend+7]);
			...
			for(k=1;k<=kpen;++k)
			{
				if( comsub[(k-1)*iend+i] >= 0.0 )
				{
					...
					else
					{
						...
						nlten_sub  = g * comsub[(k-1)*iend+i] * (  tr0[ixnumliq*mkx*iend+k*iend+i] -  tr0[ixnumliq*mkx*iend+(k-1)*iend+i] ) / ( p0[k-1] - p0[k] );
						niten_sub  = g * comsub[(k-1)*iend+i] * (  tr0[ixnumice*mkx*iend+k*iend+i] -  tr0[ixnumice*mkx*iend+(k-1)*iend+i] ) / ( p0[k-1] - p0[k] );
					}
				}
				else
				{
					...
					else
					{
						...
						nlten_sub  = g * comsub[(k-1)*iend+i] * (  tr0[ixnumliq*mkx*iend+(k-1)*iend+i] -  tr0[ixnumliq*mkx*iend+(k-2)*iend+i] ) / ( p0[k-2] - p0[k-1] );
						niten_sub  = g * comsub[(k-1)*iend+i] * (  tr0[ixnumice*mkx*iend+(k-1)*iend+i] -  tr0[ixnumice*mkx*iend+(k-2)*iend+i] ) / ( p0[k-2] - p0[k-1] );
					}
				}
				...
				nlten_sink[(k-1)*iend+i] = max( nlten_sub, - tr0[ixnumliq*mkx*iend+(k-1)*iend+i] / dt ); 
				niten_sink[(k-1)*iend+i] = max( niten_sub, - tr0[ixnumice*mkx*iend+(k-1)*iend+i] / dt );
			}
			...
			//	printf("frc_rasn=%f dwten[k-1]=%f\n",frc_rasn,dwten[k-1]);
				...
				if(k <= kbup)
				{
					...
					nc_lm   =         - g * 0.5 * ( umf[k-1] + umf[k] ) * fdr[k-1] * tr0[ixnumliq*mkx*iend+(k-1)*iend+i];  
					nc_im   =         - g * 0.5 * ( umf[k-1] + umf[k] ) * fdr[k-1] * tr0[ixnumice*mkx*iend+(k-1)*iend+i];
				}
				else
				{
					qc_lm   = 0.0;
				...
				if(k == kbup)
				{
					...
					nc_lm   = nc_lm   - g * umf[k] * tr0[ixnumliq*mkx*iend+(k-1)*iend+i] / ( ps0[k-1] - ps0[k] ); //! [ kg/kg/s ]
					nc_im   = nc_im   - g * umf[k] * tr0[ixnumice*mkx*iend+(k-1)*iend+i] / ( ps0[k-1] - ps0[k] ); //! [ kg/kg/s ]
				}
				...
				if(k == kbup)
				{
					...
					nc_lm   = nc_lm   - g * emf[k] * ( nl_emf_kbup - tr0[ixnumliq*mkx*iend+(k-1)*iend+i] ) / ( ps0[k-1] - ps0[k] ); //! [ kg/kg/s ]
					nc_im   = nc_im   - g * emf[k] * ( ni_emf_kbup - tr0[ixnumice*mkx*iend+(k-1)*iend+i] ) / ( ps0[k-1] - ps0[k] ); //! [ kg/kg/s ]
				}
				...
				if(use_expconten)
				{
					...
					else
					{
						...
						trten[ixnumliq*mkx*iend+(k-1)*iend+i] = max( nc_lm + nlten_sink[(k-1)*iend+i], - tr0[ixnumliq*mkx*iend+(k-1)*iend+i] / dt );
						trten[ixnumice*mkx*iend+(k-1)*iend+i] = max( nc_im + niten_sink[(k-1)*iend+i], - tr0[ixnumice*mkx*iend+(k-1)*iend+i] / dt );
					}
				}
				else
				{
	...
	*status = 0;
	...
	else
	{
		...
		else
		{
			...
			{
				q = -0.5*(b + fabs(1.0)*(b/(fabs(b)))*sqrt(fabs(pow(b,2) - 4.0*a*c)));//sign(1.0,b)*sqrt(pow(b,2) - 4.0*a*c));
				*r1 = q/a;
				*r2 = c/q;
			}
	...
	double *cnt_inv  ,double *cnb_inv    ,int lchnk_c     ,double *dpdry0_inv )
{
	...
	// for(int tempi=0;tempi<768;++tempi)
	// {
...
//	//hipHostMalloc((void**) &snow_c,pcols*sizeof(double));
	...
	double tw0_c[pver*pcols];
	double qw0_c[pver*pcols];
///////output/////////
...
//ret4 = GPTLstop("CPU computing2");
	...
	float elapsedTime,uwshcutime=0.0,chuanshutime1=0.0,chuanshutime2=0.0;
	int deviceCount = 0;
	hipGetDeviceCount(&deviceCount);
...
for(int tempi=myid;tempi<15120000;tempi+=size)
{
hipDeviceSynchronize();
...
hipEvent_t nstart2222,nstop2222;
hipEventCreate(&nstart2222);
hipEventCreate(&nstop2222);
hipEventRecord(nstart2222);
...
hipDeviceSynchronize();
hipEventRecord(nstop2222);
		
hipEventSynchronize(nstop2222);

hipEventElapsedTime(&elapsedTime, nstart2222, nstop2222);
chuanshutime1+=elapsedTime;
//for(int tempi=myid;tempi<3072;tempi+=size)
//{
...
hipEventRecord(nstart11);
		hipLaunchKernelGGL(compute_uwshcu, dim3(dim3(ceil(ncol/128.0))), dim3(dim3(128)), 0, 0, 
...
if(false)
{
	...
	for(i=0;i<16;++i)
		printf("i=%d,cush=%15.12e\n",i,cush[i]);
	for(i=0;i<52;++i)
		printf("umf_inv=%15.12e\n",umf_inv[i*pcols+220]);
	for(i=0;i<51;++i)
		printf("trten_inv=%15.12e\n",trten_inv[9*pcols*pver+i*pcols+220]);
}
