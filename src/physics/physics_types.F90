...
!#if ((defined wrf) || (defined geatm))  !zmh
...
!---------------------------Local storage-------------------------------
    ...
    integer :: ixnumice, ixnumliq, ixnum4          ! added by fkc
    ...
    ! added by fkc
    call cnst_get_ind('num_a4', ixnum4, abort=.false.) 
    do m = 1, pcnst
       if(ptend%lq(m)) then
          do k = ptend%top_level, ptend%bot_level
             do i = 1,ncol
          ...
          ! don't call qneg3 for number concentration variables and num4, added by fkc
          name = trim(ptend%name) // '/' // trim(cnst_name(m))
          if (m .eq. ixnum4) then
             call qneg3_modalx1(trim(name), state%lchnk, ncol, pcols, pver, m, m, qmin(m), state%q(1,1,m),1.0_r8)
          else if (m .ne. ixnumice  .and.  m .ne. ixnumliq) then
             name = trim(ptend%name) // '/' // trim(cnst_name(m))
