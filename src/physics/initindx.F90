...
!-----------------------------------------------------------------------
  ...
  use geatm_aerosol_cycle,only: geatm_aerosol_register ! juanxiong he
#if ( defined WACCM_PHYS )
  use exbdrift,           only: exbdrift_register
!czy20181116  use gw_drag,            only: gw_drag_register
#endif
!+czy20181116
  use gw_drag,            only: gw_drag_scheme
  use gw_drag_waccm,      only: gw_drag_register
...
!
  ...
  character(len=*), parameter :: subname = 'initindx' !czy20181116
!-----------------------------------------------------------------------
...
#ifdef geatm
      ! register GEATM, juanxiong he
      call geatm_aerosol_register()
#endif
...
!czy20181116      call gw_drag_register()
#endif
!+czy20181116
      if (gw_drag_scheme == 2) then
        call gw_drag_register()         !waccm gw drag scheme
      elseif (gw_drag_scheme /= 1 .and. gw_drag_scheme /= 3) then
