...
subroutine diag_init
  ...
  ! Declare the history fields for which this module contains outfld calls.
   ...
   !---------------------------------------------
#ifdef geatm
   call addfld ('dust01  ','ug/m3   ',pver, 'A','dust01' ,phys_decomp)
   call addfld ('dust02  ','ug/m3   ',pver, 'A','dust02',phys_decomp)
   call addfld ('dust03  ','ug/m3   ',pver, 'A','dust03',phys_decomp)
   call addfld ('dust04  ','ug/m3   ',pver, 'A','dust04' ,phys_decomp)
   call addfld ('sea01   ','ug/m3   ',pver, 'A','sea01' ,phys_decomp)
   call addfld ('sea02   ','ug/m3   ',pver, 'A','sea02',phys_decomp)
   call addfld ('sea03   ','ug/m3   ',pver, 'A','sea03',phys_decomp)
   call addfld ('sea04   ','ug/m3   ',pver ,'A','sea04' ,phys_decomp)
   call addfld ('ppm     ','ug/m3   ',pver ,'A','ppm' ,phys_decomp)
   call addfld ('bc      ','ug/m3   ',pver ,'A','bc' ,phys_decomp)
   call addfld ('oc      ','ug/m3   ',pver ,'A','oc' ,phys_decomp)
   call addfld ('so4     ','ug/m3   ',pver ,'A','so4' ,phys_decomp)
   call addfld ('nh4     ','ug/m3   ',pver ,'A','nh4' ,phys_decomp)
   call addfld ('h2o2    ','ug/m3   ',pver ,'A','h2o2' ,phys_decomp)
   call addfld ('so2     ','ug/m3   ',pver ,'A','so2' ,phys_decomp)
   call addfld ('no3     ','ug/m3   ',pver ,'A','no3' ,phys_decomp)
   call addfld ('nh3     ','ug/m3   ',pver ,'A','nh3' ,phys_decomp)
   call addfld ('dms     ','ug/m3   ',pver ,'A','dms' ,phys_decomp)
#endif
#ifdef CO2
   ...
   !---------------------------------------------
#ifdef geatm
   call add_default ('dust01  ',1,' ')
   call add_default ('dust02  ',1,' ')
   call add_default ('dust03  ',1,' ')
   call add_default ('dust04  ',1,' ')
   call add_default ('sea01   ',1,' ')
   call add_default ('sea02   ',1,' ')
   call add_default ('sea03   ',1,' ')
   call add_default ('sea04   ',1,' ')
   call add_default ('ppm     ',1,' ')
   call add_default ('bc      ',1,' ')
   call add_default ('oc      ',1,' ')
   call add_default ('so4     ',1,' ')
   call add_default ('nh4     ',1,' ')
   call add_default ('h2o2    ',1,' ')
   call add_default ('so2     ',1,' ')
   call add_default ('no3     ',1,' ')
   call add_default ('nh3     ',1,' ')
   call add_default ('dms     ',1,' ')
#endif
#ifdef CO2
...
!-----------------------------------------------------------------------
    ...
    use geatm_aerosol_cycle,  only: gptr,g_names ! juanxiong he
...
#ifdef geatm
    ! juanxiong he   
    real(r8), pointer, dimension(:,:) :: dust01    ! dust01
    real(r8), pointer, dimension(:,:) :: dust02    ! dust02
    real(r8), pointer, dimension(:,:) :: dust03    ! dust03
    real(r8), pointer, dimension(:,:) :: dust04    ! dust04
    real(r8), pointer, dimension(:,:) :: sea01    ! sea01
    real(r8), pointer, dimension(:,:) :: sea02    ! sea02
    real(r8), pointer, dimension(:,:) :: sea03    ! sea03
    real(r8), pointer, dimension(:,:) :: sea04    ! sea04
    real(r8), pointer, dimension(:,:) :: ppm    ! ppm
    real(r8), pointer, dimension(:,:) :: bc    ! bc
    real(r8), pointer, dimension(:,:) :: oc    ! oc
    real(r8), pointer, dimension(:,:) :: so4    ! so4
    real(r8), pointer, dimension(:,:) :: nh4    ! nh4
    real(r8), pointer, dimension(:,:) :: no3    ! no3
    real(r8), pointer, dimension(:,:) :: h2o2    ! h2o2
    real(r8), pointer, dimension(:,:) :: so2    ! so2
    real(r8), pointer, dimension(:,:) :: nh3    ! nh3
    real(r8), pointer, dimension(:,:) :: dms    ! dms
    integer :: aidx
#endif
#ifdef CO2
    integer :: aidx
...
#endif
#ifdef geatm
    aidx=pbuf_get_fld_idx(g_names(1))
    dust01 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('dust01', dust01,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(2))
    dust02 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('dust02', dust02,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(3))
    dust03 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('dust03', dust03,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(4))
    dust04 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('dust04', dust04,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(5))
    sea01 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('sea01', sea01,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(6))
    sea02 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('sea02', sea02,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(7))
    sea03 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('sea03', sea03,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(8))
    sea04 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('sea04', sea04,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(9))
    ppm => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('ppm', ppm,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(10))
    bc => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('bc', bc,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(11))
    oc => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('oc', oc,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(12))
    nh4 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('nh4', nh4,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(13))
    no3 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('no3', no3,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(14))
    so4 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('so4', so4,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(15))
    h2o2 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('h2o2', h2o2,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(16))
    so2 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('so2', so2,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(17))
    dms => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('dms', dms,  pcols, lchnk)

    aidx=pbuf_get_fld_idx(g_names(18))
    nh3 => pbuf(aidx)%fld_ptr(1,1:pcols,1:pver,lchnk,1)
    call outfld('nh3', nh3,  pcols, lchnk)
