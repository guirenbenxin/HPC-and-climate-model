 ...
 ! use spmd_utils,     only : iam
  ...
  real(r8)            :: estbl(1:250)       !  Environmental pressure at the interfaces [ Pa ]
  real(r8)            :: qmin(1:25)       !  Environmental height at the interfaces   [ m ]
  integer             :: numptr_amode(1:3)       !  Environmental height at the interfaces   [ m ]
...
!===============================================================================
  ...
  subroutine compute_uwshcu_inv( mix      , mkx        , iend          , ncnst     , dt       ,  & 
                                 ...
                                 cbmf     , qc_inv     , rliq          ,                         &
                                 cnt_inv  , cnb_inv    , lchnk     , dpdry0_inv , node, np, ierr)
    use mpi
    implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    xlv      =       2501000.00000000_r8                
    xlf      =       333700.000000000_r8              
    xls      =       2834700.000000000_r8              
    cp       =       1004.64000000000_r8               
    zvir     =       0.607793072824156_r8              
    r        =       287.042311365049_r8               
    g        =       9.80616000000000_r8               
    ep2      =       0.621970586204516_r8                
    p00      =       1.0e5_r8               
    rovcp    =       1.2857165864041_r8              
    rpen     =       10.0000000000_r8          


    tmin     =       173.1599999999999965894_r8                   
    tmax     =       375.1600000000000250111_r8                
    ttrice   =       20.0000000000000000000_r8                  
    epsqs    =       0.6219705862045155076_r8                 
    rgasv    =       461.5046398201599231470_r8               
    hlatf    =       333700.0000000000000000000_r8                   
    hlatv    =       2501000.0000000000000000000_r8                   
    tmelt    =       273.1499999999999772626_r8              
    icephs   =       .false.
...
! do k = 1, np-1
!    write(*,*) 'k=',k
!    call MPI_ISEND(trten,mix*mkx*ncnst,MPI_DOUBLE_PRECISION,k,99,MPI_COMM_WORLD,request,ierr)
! enddo
!endif
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
call CPU_TIME(t1)
!if(node .ne. 0) then
do k = node+1, 18432,np
    call compute_uwshcu( mix  , mkx    , iend      , ncnst , dt   , &
                         ...
                         cufrc, qcu    , qlu       , qiu   ,        &
                         cbmf , qc     , rliq      ,                &
                         cnt  , cnb    , lchnk , dpdry0, node, np, ierr )
enddo 
!endif
!call MPI_BARRIER(MPI_COMM_WORLD, ierr)

call CPU_TIME(t2)
write (*,*) 'uwshcu time=',t2-t1
...
!!!    write(*,*) 'cnt_inv(2)',cnt_inv(2)
!!!    write(*,*) 'cnb_inv(2)',cnb_inv(2)

!!!    write(*,*) 'umf_inv(2,34)',umf_inv(2,34)
!!!    write(*,*) 'slflx_inv(2,34)',slflx_inv(2,34)
!!!    write(*,*) 'qtflx_inv(2,34)',qtflx_inv(2,34)
!!!    write(*,*) 'flxprc1_inv(2,34)',flxprc1_inv(2,34)
!!!    write(*,*) 'flxsnow1_inv(2,34)',flxsnow1_inv(2,34)

!!!    write(*,*) 'qvten_inv(2,34)',qvten_inv(2,34)
!!!    write(*,*) 'qlten_inv(2,34)',qlten_inv(2,34)
!!!    write(*,*) 'qiten_inv(2,34)',qiten_inv(2,34)
!!!    write(*,*) 'sten_inv(2,34)',sten_inv(2,34)
!!!    write(*,*) 'uten_inv(2,34)',uten_inv(2,34)
!!!    write(*,*) 'vten_inv(2,34)',vten_inv(2,34)
!!!    write(*,*) 'qrten_inv(2,34)',qrten_inv(2,34)
!!!    write(*,*) 'qsten_inv(2,34)',qsten_inv(2,34)
!!!    write(*,*) 'evapc_inv(2,34)',evapc_inv(2,34)
!!!    write(*,*) 'cufrc_inv(2,34)',cufrc_inv(2,34)
!!!    write(*,*) 'qcu_inv(2,34)',qcu_inv(2,34)
!!!    write(*,*) 'qlu_inv(2,34)',qlu_inv(2,34)
!!!    write(*,*) 'qiu_inv(2,34)',qiu_inv(2,34)
!!!    write(*,*) 'qc_inv(2,34)',qc_inv(2,34)

!!!    write(*,*) 'trten_inv(2,34,21)',trten_inv(2,34,21)
...
!======================== zhh debug 2012-02-09 =======================     
  ...
  subroutine compute_uwshcu( mix      , mkx       , iend         , ncnst    , dt        , &
                             ...
                             cnt_out  , cnb_out   , lchnk    , dpdry0_in, node, np, ierr)
...
!   parameter (rkm = 16.0_r8)        !  Determine the amount of air that is involved in buoyancy-sorting [no unit] 
   ...
   ! call cnst_get_ind( 'NUMICE', ixnumice )
    ...
    ! ------------------------------------------------------- !
!    write(500+iam,*)'xlv=',xlv
!    write(500+iam,*)'xlf=',xlf
!    write(500+iam,*)'xls=',xls
!    write(500+iam,*)'cp=',cp
!    write(500+iam,*)'zvir=',zvir
!    write(500+iam,*)'r=',r
!    write(500+iam,*)'g=',g
!    write(500+iam,*)'ep2=',ep2
!    write(500+iam,*)'p00=',p00
!    write(500+iam,*)'rovcp=',rovcp
...
!======================== zhh debug 2012-02-09 =======================     
    ...
    tw0_in(:iend,:mkx)  = 0.0_r8
    qw0_in(:iend,:mkx)  = 1.0_r8
...
!!!if(iter .eq. 1) write(*,*) 'plcl=',plcl
 ...
 35    continue
     ...
     !                'cinlfc'.
       ...
       if( use_CINcin ) then       
           wcrit = sqrt(abs( 2._r8 * cin * rbuoy ))      
       else
           wcrit = sqrt(abs( 2._r8 * cinlcl * rbuoy ))   
       endif
       sigmaw = sqrt(abs( rkfre * tkeavg + epsvarw ))
       ...
       mumin0 = 0._r8
       if( cbmf .gt. cbmflimit ) mumin0 = sqrt(abs(-log(abs(2.5066_r8*cbmflimit/rho0inv/sigmaw))))
       ! 2. 'ufrcinv' constraint
       mu = max(max(mu,mumin0),mumin1)
       ! 3. 'ufrclcl' constraint      
       mulcl = sqrt(abs(2._r8*cinlcl*rbuoy))/1.4142_r8/sigmaw
       mulclstar = sqrt(abs(max(0._r8,2._r8*(exp(-mu**2)/2.5066_r8)**2*(1._r8/1.1**2-0.25_r8/rmaxfrac**2))))
       if( mulcl .gt. 1.e-8_r8 .and. mulcl .gt. mulclstar ) then
           mumin2 = compute_mumin2(mulcl,rmaxfrac,mu)
           if( mu .gt. mumin2 ) then
            !   write(*,*) 'Critical error in mu calculation in UW_ShCu'
            !   call endrun
                return;
           endif
           mu = max(mu,mumin2)
       ...
       cbmf = (rho0inv*sigmaw/2.5066_r8)*exp(-mu**2) 
       winv = sigmaw*(2._r8/2.5066_r8)*exp(-mu**2)/1.1
       ufrcinv = cbmf/winv/rho0inv
!write(*,*)'i=',i,'winv=',winv,'mu=',mu,'test=',erfc(1.0_r8)
       ! ------------------------------------------------------------------- !
     ...
     ! OK
       ...
       do iter_xc = 1, niter_xc
          ...
          qsat_arg = thle*exne     
          !------------------------!
          qsat_arglf(1) = qsat_arg
          pelf(1) = pe

          status   = qsat(qsat_arglf(1),pelf(1),es(1),qs(1),gam(1),1)
          excess0  = qte - qs(1)
        ...
        ! rei(k) = ( rkm / scaleh / g / rho0j )        ! Default.
          ...
          if( xc .gt. 0.5_r8 ) rei(k) = min(rei(k),0.9_r8*log(abs(dp0(k)/g/dt/umf(km1) + 1._r8))/dpe/(2._r8*xc-1._r8))
          fer(k) = rei(k) * ee2
          fdr(k) = rei(k) * ud2
        ...
        ! endif 
          ...
          if( wtw .gt. 0._r8 ) then   
              ...
              wue   = 0.5_r8 *   sqrt(abs( max( wtwb + wtw, 0._r8 ) ))
          else
              go to 111
          endif 
   ...
   111 continue
        ...
        ! if( bogtop .gt. 0._r8 ) then          
          ...
          if( ufrc(k) .gt. rmaxfrac ) then
              ...
              fdr(k)  = fer(k) - log( abs(umf(k) / umf(km1) )) / dpe
          endif
 ...
 45    continue
       ...
       if( drage .eq. 0._r8 ) then
           ...
           if( status .eq. 0 ) then
               ...
               elseif( xc1 .gt. 0._r8 .and. xc2 .gt. 0._r8 ) then
                   ppen = -dp0(kpen)
                   write(*,*) 'Warning : UW-Cumulus penetrates upto kpen interface'
               else
                   ppen = min( xc1, xc2 )
                   ppen = min( 0._r8,max( -dp0(kpen), ppen ) )  
               endif
       ...
       do m = 1, ncnst
          ...
          call fluxbelowinv( cbmf, ps0(0:mkx), mkx, kinv, dt, xsrc, xmean, xtop, xbot, xflx )
   ! if((i .eq. 8) .and. (m .eq. 17)) write(*,*)'kinv=',kinv,'cbmf=',cbmf,'dt=',dt,'xsrc=',xsrc,'xmean=',xmean,'xtop=',xtop,'xbot=',xbot
   ! if((i .eq. 8) .and. (m .eq. 17)) write(*,*)'xflx=',xflx(0:kinv-1)
...
!if(i .eq. 8) then
!write(*,*)'emf(1)=',emf(1),'tru_emf(1,17)=',tru_emf(1,17),'tr0(1,17)=',tr0(1,17),'sstr0(1,17)=',sstr0(1,17),'ps0(1)=',ps0(1),'p0(1)=',p0(1)
!write(*,*) 'trflx=',trflx(1,17)
!endif
       ! ------------------------------------------- !
       ...
       do k = mkx, 1, -1  ! 'k' is a layer index : 'mkx'('1') is the top ('bottom') layer
          ...
          evprain  = kevp * subsat * sqrt(abs(flxrain(k)+snowmlt*dp0(k)/g)) 
          evpsnow  = kevp * subsat * sqrt(abs(max(flxsnow(k)-snowmlt*dp0(k)/g,0._r8)))
...
!======================== zhh debug 2012-02-09 =======================     
 ...
 333 if(.false.) then ! Exit without cumulus convection
...
!======================== zhh debug 2012-02-09 =======================   
    ...
    if( a .eq. 0._r8 ) then                            ! Form b*x + c = 0
        ...
        if( b .eq. 0._r8 ) then                        ! Form a*x**2 + c = 0
            ...
            if( (b**2 - 4._r8*a*c) .lt. 0._r8 ) then   ! Failure, no real roots
                 ...
                 q  = -0.5_r8*(b + sign(1.0_r8,b)*sqrt(abs(b**2 - 4._r8*a*c)))
                 r1 =  q/a
                 r2 =  c/q
            endif
       endif
  ...
  ! ----------------------------------------------------------------- !
    ...
    if( rhi .le. 0.01_r8 ) then
        write(*,*) 'Source air is too dry and pLCL is set to psmin in uwshcu.F90' 
        qsinvert = psmin
        return
    end if
    TLCL     =  55._r8 + 1._r8/(1._r8/(Ti-55._r8)-log(abs(rhi))/2840._r8); ! Bolton's formula. MWR.1980.Eq.(22)
    PiLCL    =  TLCL/thl
    ...
    do i = 1, 10
       ...
       ps       =  ps + dps
       if( ps .lt. 0._r8 ) then
       !    write(*,*) 'pLCL iteration is negative and set to psmin in uwshcu.F90', qt, thl, psfc 
           qsinvert = psmin
           return    
       end if
 ...
 !   write(*,*) 'pLCL does not converge and is set to psmin in uwshcu.F90', qt, thl, psfc 
    qsinvert = psmin
    return
 ...
 20 return
  ...
  ! ------------------------------------------------------------------------------- !
    ...
    if( dqv .gt. 1.e-20_r8 ) then
        ...
        !    write(*,*) 'Full positive_moisture is impossible in uwshcu'
        endif
    endif 
...
10    continue
      ...
      if (dtm > dttol .or. dqm > dqtol) then
         ...
         if (error_found) then
            do i = 1,ncol
               if (doit(i) == 0) then
             !     write(*,*) ' findsp not converging at point i, k ', i, k
             !     write(*,*) ' t, q, p, enin ', t(i,k), q(i,k), p(i,k), enin(i)
             !     write(*,*) ' tsp, qsp, enout ', tsp(i,k), qsp(i,k), enout(i)
               !   call endrun ('FINDSP')
               endif
            end do
         endif
      ...
      if (error_found) then
         do i = 1,ncol
            if (doit(i) == 2 .and. abs((enin(i)-enout(i))/(enin(i)+enout(i))) > 1.e-4_r8) then
            !   write(*,*) ' the enthalpy is not conserved for point ', &
            !      i, k, enin(i), enout(i)
            !   write(*,*) ' t, q, p, enin ', t(i,k), q(i,k), p(i,k), enin(i)
            !   write(*,*) ' tsp, qsp, enout ', tsp(i,k), qsp(i,k), enout(i)
            !  call endrun ('FINDSP')
...
!
   ...
   !-------------------------------------------------------------------
integer function qsat(t    ,p    ,es    ,qs   ,gam   , len     )
  !----------------------------------------------------------------------- 
  ! Purpose: 
...
END FUNCTION erfc
  ...
  program main
   ...
   integer , parameter :: r8 = selected_real_kind(12)    !  8 byte real
   real(r8), parameter :: unset_r8 = huge(1.0_r8)
   integer :: pcols, pver, ncol, pcnst, lchnk, i, j, k
   real(r8) :: ztodt
  
    real(r8) :: ps0_inv(1152,0:30)       !  Environmental pressure at the interfaces [ Pa ]
    real(r8) :: zs0_inv(1152,0:30)       !  Environmental height at the interfaces   [ m ]
    real(r8) :: p0_inv(1152,30)          !  Environmental pressure at the layer mid-point [ Pa ]
    real(r8) :: z0_inv(1152,30)          !  Environmental height at the layer mid-point [ m ]
    real(r8) :: dp0_inv(1152,30)         !  Environmental layer pressure thickness [ Pa ] > 0.
    real(r8) :: dpdry0_inv(1152,30)      !  Environmental dry layer pressure thickness [ Pa ]
    real(r8) :: u0_inv(1152,30)          !  Environmental zonal wind [ m/s ]
    real(r8) :: v0_inv(1152,30)          !  Environmental meridional wind [ m/s ]
    real(r8) :: qv0_inv(1152,30)         !  Environmental water vapor specific humidity [ kg/kg ]
    real(r8) :: ql0_inv(1152,30)         !  Environmental liquid water specific humidity [ kg/kg ]
    real(r8) :: qi0_inv(1152,30)         !  Environmental ice specific humidity [ kg/kg ]
    real(r8) :: t0_inv(1152,30)          !  Environmental temperature [ K ]
    real(r8) :: s0_inv(1152,30)          !  Environmental dry static energy [ J/kg ]
    real(r8) :: tr0_inv(1152,30,25)   !  Environmental tracers [ #, kg/kg ]
    real(r8) :: tke_inv(1152,0:30)       !  Turbulent kinetic energy at the interfaces [ m2/s2 ]
    real(r8) :: cldfrct_inv(1152,30)     !  Total cloud fraction at the previous time step [ fraction ]
    real(r8) :: concldfrct_inv(1152,30)  !  Total convective ( shallow + deep ) cloud fraction at the previous time step [ fraction ]
    real(r8) :: pblh(1152)                !  Height of PBL [ m ]
    real(r8) :: cush(1152)                !  Convective scale height [ m ]
    real(r8) :: umf_inv(1152,0:30)       !  Updraft mass flux at the interfaces [ kg/m2/s ]
    real(r8) :: qvten_inv(1152,30)       !  Tendency of water vapor specific humidity [ kg/kg/s ]
    real(r8) :: qlten_inv(1152,30)       !  Tendency of liquid water specific humidity [ kg/kg/s ]
    real(r8) :: qiten_inv(1152,30)       !  Tendency of ice specific humidity [ kg/kg/s ]
    real(r8) :: sten_inv(1152,30)        !  Tendency of dry static energy [ J/kg/s ]
    real(r8) :: uten_inv(1152,30)        !  Tendency of zonal wind [ m/s2 ]
    real(r8) :: vten_inv(1152,30)        !  Tendency of meridional wind [ m/s2 ]
    real(r8) :: trten_inv(1152,30,25) !  Tendency of tracers [ #/s, kg/kg/s ]
    real(r8) :: qrten_inv(1152,30)       !  Tendency of rain water specific humidity [ kg/kg/s ]
    real(r8) :: qsten_inv(1152,30)       !  Tendency of snow specific humidity [ kg/kg/s ]
    real(r8) :: precip(1152)              !  Precipitation ( rain + snow ) flux at the surface [ m/s ]
    real(r8) :: snow(1152)                !  Snow flux at the surface [ m/s ]
    real(r8) :: evapc_inv(1152,30)       !  Evaporation of precipitation [ kg/kg/s ]
    real(r8) :: rliq(1152)                !  Vertical integral of tendency of detrained cloud condensate qc [ m/s ]
    real(r8) :: slflx_inv(1152,0:30)     !  Updraft liquid static energy flux [ J/kg * kg/m2/s ]
    real(r8) :: qtflx_inv(1152,0:30)     !  Updraft total water flux [ kg/kg * kg/m2/s ]
    real(r8) :: flxprc1_inv(1152,0:30)   ! uw grid-box mean rain+snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90
    real(r8) :: flxsnow1_inv(1152,0:30)  ! uw grid-box mean snow flux (kg m^-2 s^-1) for physics buffer calls in convect_shallow.F90

    real(r8) :: cufrc_inv(1152,30)       !  Shallow cumulus cloud fraction at the layer mid-point [ fraction ]
    real(r8) :: qcu_inv(1152,30)         !  Liquid+ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8) :: qlu_inv(1152,30)         !  Liquid water specific humidity within cumulus updraft [ kg/kg ]
    real(r8) :: qiu_inv(1152,30)         !  Ice specific humidity within cumulus updraft [ kg/kg ]
    real(r8) :: qc_inv(1152,30)          !  Tendency of cumulus condensate detrained into the environment [ kg/kg/s ]
    real(r8) :: cbmf(1152)                !  Cumulus base mass flux [ kg/m2/s ]
    real(r8) :: cnt_inv(1152)             !  Cumulus top  interface index, cnt = kpen [ no ]
    real(r8) :: cnb_inv(1152)             !  Cumulus base interface index, cnb = krel - 1 [ no ]
...
do k = 0, pver
    do i = 1, ncol
      ps0_inv(i,k)  = (k+0.5_r8)*1.0e-5_r8    
      zs0_inv(i,k)  = (k+0.5_r8)
      tke_inv(i,k)  = (k+0.5_r8)*1.0e-5_r8
    enddo
enddo

do k = 1, pver
    do i = 1, ncol
    p0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    z0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    dp0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    dpdry0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    u0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    v0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    qv0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    ql0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    qi0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    t0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    s0_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    cldfrct_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    concldfrct_inv(i,k) = (i+k+0.1_r8)*1.0e-0_r8
    enddo
enddo
...
do i = 1, pcnst
    do k = 1, pver
        do j = 1, ncol
        tr0_inv(j,k,i) = (k+j+2._r8)*1.0e-0_r8
        enddo
    enddo
enddo
...
do k = 0, pver
    ...
    slflx_inv(i,k) = 1._r8
    qtflx_inv(i,k) = 1._r8
    flxprc1_inv(i,k) = 1._r8
    flxsnow1_inv(i,k) = 1._r8
    enddo
enddo
...
do k = 1, pcnst
    do j = 1, pver
        do i = 1, ncol
!        if(tr0_inv(i,j,k) .ne. 0._r8) write(*,*)'i=',i,'j=',j,'k',k,'tr0_inv=',tr0_inv(i,j,k)
        enddo
    enddo
enddo
...
!!!do k = 0, pver
!!!    do i = 1, ncol
!!!    if(cufrc_inv(i,k) .ne. 0._r8) then
!!!          write(*,*) 'i=',i,'snow=',snow(i)
!!!    endif
!!!    enddo
!!!enddo
!!!write(*,*)'lifei=',min(1._r8,log(-1._r8))

if(node .eq. 0) then
    write(*,*)'cush=',cush(:16)
    write(*,*)'umf_inv=',umf_inv(221,0:pver)
    write(*,*)'trten_inv=',trten_inv(221,:pver,10)
endif
