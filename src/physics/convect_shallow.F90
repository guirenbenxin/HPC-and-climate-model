...
!zmh
  ...
  case('UW') ! Park and Bretherton shallow convection scheme
     ...
     if( .not. (eddy_scheme .eq. 'diag_TKE' .or. eddy_scheme .eq. 'MTKE' )) then
...
!zmh
  ...
  subroutine convect_shallow_tend( ztodt  , tpert   , qpert   , gradt, gradq, pblht , & 
                                   cmfmc  , cmfmc2  , precc    , &
                                   qc     , qc2     , rliq     , rliq2    , & 
...
call t_startf('compute_uwshcu_inv')
...
!      end if
!======================== zhh debug 2012-02-09 =======================
call t_stopf('compute_uwshcu_inv')
      ! --------------------------------------------------------------------- !
...
!wxc zmh
      ...
      call compute_zwshcu_inv( pcols     , pver    , ncol           , pcnst         , ztodt         ,                   &
                               ...
                               tke       , cld     , concld         , pblht         , cush          ,                   &
!zmh added input
                               state%omega,tpert  , qpert(:,1), gradt, gradq, &
                               landfrac, state%lat, state%lon , massflx_sh, &
                               cmfmc2    , slflx   , qtflx          , 							&
			       flxprec, flxsnow, 			         					&
                               ptend_loc%q(:,:,1)  , ptend_loc%q(:,:,ixcldliq), ptend_loc%q(:,:,ixcldice),              &
                               ptend_loc%s         , ptend_loc%u    , ptend_loc%v   , ptend_tracer  ,                   &
