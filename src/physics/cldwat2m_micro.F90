...
! zmh 2
  ...
  use cloud_fraction, only: microp_scheme,varv_flag, disp_flagr,disp_flagc,ipgam,kk_flag,&
                            zmh_berg,zmh_taui,zmh_ac, zmh_ai,zmh_kk
...
! density parameters (kg/m3)
      ...
      !zmh rhosn = 350._r8    ! bulk density snow  (++ ceh)
...
!zmh 1, 2, 3
 ...
 ! hm switch for sub-columns, don't include sub-grid qc
           ...
           if (sub_column) then
              ...
              ! (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8) * W11     * zmh_kk
! Modified by Wu Chenglai, 2019-12-08
              prc(k) = enhance6(k)*30500._r8*qcic(i,k)**3.19_r8* &
               (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.2_r8) * W11     * zmh_kk
              ...
              ! (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.79_r8) * W11      * zmh_kk
              prc(k) = enhance6(k)*W12 *30500._r8*qcic(i,k)**3.19_r8* &
               (ncic(i,k)/1.e6_r8*rho(i,k))**(-1.2_r8) * W11      * zmh_kk
...
! note: assumes autoconversion timescale of 180 sec
           ...
           nprci(k) = n0i(k)/(lami(k)*180._r8)*exp(-lami(k)*dcs)/zmh_taui

           prci(k) = pi*rhoi*n0i(k)/(6._r8*180._r8)* &
              (cons23/lami(k)+3._r8*cons24/lami(k)**2+ &
          6._r8*dcs/lami(k)**3+6._r8/lami(k)**4)*exp(-lami(k)*dcs)/zmh_taui          
