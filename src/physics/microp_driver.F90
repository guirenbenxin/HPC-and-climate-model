...
!    use cldwat2m_macro,  only: ini_macro
    use cldwat2m_macro_pbl,  only: ini_macro
...
! Retrieve the indices for pbuf entries
  ...
  !use cldwat2m_micro,   only: mmicro_pcond     !zmh
     ...
     ! ----------------------- !
       ...
       do m = 1, naer_all
          ...
          if( is_first_step() ) then
              ...
              write(iulog,*)'calling microp new_rad_cnst'
...
!              aer_mmr(:ncol,:,m) = aermmr1(:ncol,:)
      ...
      call mmicro_pcond( sub_column, lchnk, ncol, dtime, state1%t,                  &  !zmh
#if (! defined BFB_CAM_SCAM_IOP )
                         state1%lat, state1%zm,                                    &  !zmh 2  not ulat
#endif
                         state1%q(1,1,1), qc, qi,                                   &
                         nc, ni, state1%pmid, state1%pdel, ast,                     &
                         alst_mic, aist_mic,                                        &
