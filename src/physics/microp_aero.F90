...
!  use cldwat2m_macro, only: rhmini, rhmaxi
  use cldwat2m_macro_pbl, only: rhmini, rhmaxi
...
!!!
   ...
   wsubTKE = .false.
   if (trim(eddy_scheme) .eq. 'diag_TKE' .or. trim(eddy_scheme) .eq. 'MTKE') wsubTKE = .true.
...
!!! added by fkc, ensure never adding existing aerosols


!addfield calls for outputting aerosol number concentration
      if (.not.(((trim(aername(iaer)) == 'SULFATE').and.(a1.gt.0)).or. &
               ((trim(aername(iaer)) == 'OCPHI')  .and.(a2.gt.0)).or. &
               ((trim(aername(iaer)) == 'SSAM')   .and.(a3.gt.0)).or. &
               ((trim(aername(iaer)) == 'DUST4')  .and.(a4.gt.0)))) then
          call addfld(trim(aername(iaer)), 'm-3', pver, 'A', 'aerosol number concentration',phys_decomp)
      end if
      if (trim(aername(iaer)) == 'SULFATE') a1 = a1+1
      if (trim(aername(iaer)) == 'OCPHI')   a2 = a2+1
      if (trim(aername(iaer)) == 'SSAM')    a3 = a3+1
      if (trim(aername(iaer)) == 'DUST4')   a4 = a4+1
...
!         internal mixture of aerosols
         ...
         !zmh if(volc(m).gt.1.e-39.and.na(m).gt.1.e-39)then
         if(volc(m).gt.1.e-19.and.na(m).gt.1.e-19)then
