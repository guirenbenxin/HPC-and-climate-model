...
//extern char constituents_mp_cnst_type_[25][3];
  double wv_saturation_mp_estbl_[250];// ! table values of saturation vapor pressure;

  double wv_saturation_mp_tmin_=173.1599999999999965894;//! min temperature (K) for table
  double wv_saturation_mp_tmax_=375.1600000000000250111;//! max temperature (K) for table
  double wv_saturation_mp_ttrice_=20.0000000000000000000;//! transition range from es over H2O to es over ice
//  double wv_saturation_mp_pcf_[6];//! polynomial coeffs -> es transition water to ice
  double wv_saturation_mp_epsqs_=0.6219705862045155076;//! Ratio of h2o to dry air molecular weights
  double wv_saturation_mp_rgasv_=461.5046398201599231470;//! Gas constant for water vapor
  double wv_saturation_mp_hlatf_=333700.0000000000000000000;//! Latent heat of vaporization
  double wv_saturation_mp_hlatv_=2501000.0000000000000000000;//! Latent heat of fusion
  double wv_saturation_mp_cp_=1004.6399999999999863576;//! specific heat of dry air
  double wv_saturation_mp_tmelt_=273.1499999999999772626;//! Melting point of water (K)
...
__device__ void positive_moisture_single(int i,double xlv,double xls,int mkxtemp,double dt,double qvmin,double qlmin,double qimin,double *dp,double *qv, 
							  			 double *ql,double *qi,double *s,double *qvten,double *qlten,double *qiten,double *sten );
__device__ double eerfc(double x);

__constant__ double xlv = 2501000.00000000;
__constant__ double xlf = 333700.000000000;
__constant__ double xls = 2834700.000000000;//xlv + xlf;
__constant__ double cp = 1004.64000000000;
__constant__ double zvir = 0.607793072824156;
__constant__ double r = 287.042311365049;
__constant__ double g = 9.80616000000000;
__constant__ double ep2 = 0.621970586204516;
__constant__ double p00 = 1.0e5;
__constant__ double rovcp = 1.2857165864041;//r/cp;
__constant__ double rpen = 10.0000000000;

__constant__  double wv_saturation_mp_tmin__=173.1599999999999965894;//! min temperature (K) for table
__constant__  double wv_saturation_mp_tmax__=375.1600000000000250111;//! max temperature (K) for table
__constant__  double wv_saturation_mp_ttrice__=20.0000000000000000000;//! transition range from es over H2O to es over ice

//__device__  double wv_saturation_mp_pcf__[6];//! polynomial coeffs -> es transition water to ice

__constant__  double wv_saturation_mp_epsqs__=0.6219705862045155076;//! Ratio of h2o to dry air molecular weights
__constant__  double wv_saturation_mp_rgasv__=461.5046398201599231470;//! Gas constant for water vapor
__constant__  double wv_saturation_mp_hlatf__=333700.0000000000000000000;//! Latent heat of vaporization
__constant__  double wv_saturation_mp_hlatv__=2501000.0000000000000000000;//! Latent heat of fusion
__constant__  double wv_saturation_mp_cp__=1004.6399999999999863576;//! specific heat of dry air
__constant__  double wv_saturation_mp_tmelt__=273.1499999999999772626;//! Melting point of water (K)
__constant__  bool   wv_saturation_mp_icephs__=false;//! false => saturation vapor press over water only

__device__  double wv_saturation_mp_estbl__[250];
...
//ret2 = GPTLstart("Malloc");
	...
	///////input//////////
	double *ps0_c;
	hipHostMalloc((double**) &ps0_c,mix*(mkx+1)*sizeof(double));
	double *zs0_c;
	hipHostMalloc((double**) &zs0_c,mix*(mkx+1)*sizeof(double));
	double *p0_c;
	hipHostMalloc((double**) &p0_c,mix*mkx*sizeof(double));
	double *z0_c;
	hipHostMalloc((double**) &z0_c,mix*mkx*sizeof(double));
	double *dp0_c;
	hipHostMalloc((double**) &dp0_c,mix*mkx*sizeof(double));
	double *dpdry0_c;
	hipHostMalloc((double**) &dpdry0_c,mix*mkx*sizeof(double));
	double *u0_c;
	hipHostMalloc((double**) &u0_c,mix*mkx*sizeof(double));
	double *v0_c;
	hipHostMalloc((double**) &v0_c,mix*mkx*sizeof(double));
	double *qv0_c;
	hipHostMalloc((double**) &qv0_c,mix*mkx*sizeof(double));
	double *ql0_c;
	hipHostMalloc((double**) &ql0_c,mix*mkx*sizeof(double));
	double *qi0_c;
	hipHostMalloc((double**) &qi0_c,mix*mkx*sizeof(double));
	double *t0_c;
	hipHostMalloc((double**) &t0_c,mix*mkx*sizeof(double));
	double *s0_c;
	hipHostMalloc((double**) &s0_c,mix*mkx*sizeof(double));
	double *tr0_c;
	hipHostMalloc((double**) &tr0_c,ncnst*mix*mkx*sizeof(double));
	double *tke_c;
	hipHostMalloc((double**) &tke_c,mix*(mkx+1)*sizeof(double));
	double *cldfrct_c;
	hipHostMalloc((double**) &cldfrct_c,mix*mkx*sizeof(double));
	double *concldfrct_c;
	hipHostMalloc((double**) &concldfrct_c,mix*mkx*sizeof(double));
	////double pblh_c[pcols];
	////hipHostMalloc((void**) &pblh_c,mix*sizeof(double));
	
	///////////////////////in-out////////////////
	////double cush_c[pcols]; 
	////hipHostMalloc((void**) &cush_c,mix*sizeof(double));
	...
	double *qlten_c;
	hipHostMalloc((double**) &qlten_c,mix*mkx*sizeof(double));
	double *qiten_c;
	hipHostMalloc((double**) &qiten_c,mix*mkx*sizeof(double));
	double *sten_c;
	hipHostMalloc((double**) &sten_c,mix*mkx*sizeof(double));
	double *uten_c;
	hipHostMalloc((double**) &uten_c,mix*mkx*sizeof(double));
	double *vten_c;
	hipHostMalloc((double**) &vten_c,mix*mkx*sizeof(double));
	double *trten_c;
	hipHostMalloc((double**) &trten_c,ncnst*mix*mkx*sizeof(double));
	double *qrten_c;
	hipHostMalloc((double**) &qrten_c,mix*mkx*sizeof(double));
	double *qsten_c;
	hipHostMalloc((double**) &qsten_c,mix*mkx*sizeof(double));
//	double *precip_c[pcols];
//	//hipHostMalloc((void**) &precip_c,mix*sizeof(double));
//	double *snow_c[pcols];
//	//hipHostMalloc((void**) &snow_c,mix*sizeof(double));
	double *evapc_c;
	hipHostMalloc((double**) &evapc_c,mix*mkx*sizeof(double));
	// double *rliq_c[pcols];
	// //hipHostMalloc((void**) &rliq_c,mix*sizeof(double));slflx_c
	double *slflx_c;
	hipHostMalloc((double**) &slflx_c,mix*(mkx+1)*sizeof(double));
	double *qtflx_c;
	hipHostMalloc((double**) &qtflx_c,mix*(mkx+1)*sizeof(double));
	double *flxprc1_c;
	hipHostMalloc((double**) &flxprc1_c,mix*(mkx+1)*sizeof(double));
	double *flxsnow1_c;
	hipHostMalloc((double**) &flxsnow1_c,mix*(mkx+1)*sizeof(double));

	double *cufrc_c;
	hipHostMalloc((double**) &cufrc_c,mix*mkx*sizeof(double));
	double *qcu_c;
	hipHostMalloc((double**) &qcu_c,mix*mkx*sizeof(double));
	double *qlu_c;
	hipHostMalloc((double**) &qlu_c,mix*mkx*sizeof(double));
	double *qiu_c;
	hipHostMalloc((double**) &qiu_c,mix*mkx*sizeof(double));
	double *qc_c;
	hipHostMalloc((double**) &qc_c,mix*mkx*sizeof(double));
	// double *cbmf_c[pcols];
	// //hipHostMalloc((void**) &cbmf_c,mix*sizeof(double));
	double *cnt_c;
	hipHostMalloc((double**) &cnt_c,mix*sizeof(double));
	double *cnb_c;
	hipHostMalloc((double**) &cnb_c,mix*sizeof(double));
	...
	hipHostMalloc((void**) &snow_c,mix*sizeof(double));
	double *rliq_c;
	hipHostMalloc((void**) &rliq_c,mix*sizeof(double));
	double *cbmf_c;
	hipHostMalloc((void**) &cbmf_c,mix*sizeof(double));
	...
	double tw0_c[mkx*mix];
	double qw0_c[mkx*mix];
...
hipMalloc((void**) &tw0_in,mix*mkx*sizeof(double));

hipMalloc((void**) &qw0_in,mix*mkx*sizeof(double));

// int *lchnk_d,*ncol_d;


/////input//////////
hipMalloc((void**) &ps0_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &zs0_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &p0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &z0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &dp0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &dpdry0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &u0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &v0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qv0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &ql0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qi0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &t0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &s0_d,mix*mkx*sizeof(double));

hipMalloc((void**) &tr0_d,ncnst*mix*mkx*sizeof(double));

hipMalloc((void**) &tke_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &cldfrct_d,mix*mkx*sizeof(double));

hipMalloc((void**) &concldfrct_d,mix*mkx*sizeof(double));

hipMalloc((void**) &pblh_d,mix*sizeof(double));

// 	///////////////////////in-out////////////////

hipMalloc((void**) &cush_d,mix*sizeof(double));
...
hipMalloc((void**) &qlten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qiten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &sten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &uten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &vten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &trten_d,ncnst*mix*mkx*sizeof(double));

hipMalloc((void**) &qrten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qsten_d,mix*mkx*sizeof(double));

hipMalloc((void**) &precip_d,mix*sizeof(double));

hipMalloc((void**) &snow_d,mix*sizeof(double));

hipMalloc((void**) &evapc_d,mix*mkx*sizeof(double));

hipMalloc((void**) &rliq_d,mix*sizeof(double));

hipMalloc((void**) &slflx_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &qtflx_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &flxprc1_d,mix*(mkx+1)*sizeof(double));

hipMalloc((void**) &flxsnow1_d,mix*(mkx+1)*sizeof(double));


hipMalloc((void**) &cufrc_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qcu_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qlu_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qiu_d,mix*mkx*sizeof(double));

hipMalloc((void**) &qc_d,mix*mkx*sizeof(double));

hipMalloc((void**) &cbmf_d,mix*sizeof(double));

hipMalloc((void**) &cnt_d,mix*sizeof(double));

hipMalloc((void**) &cnb_d,mix*sizeof(double));

hipMalloc((void**) &qmin,ncnst*sizeof(double));
...
double *trten_s;
double *tr0_o;
double *sstr0_o;  //trten_o[mix*mkx*ncnst], 
//double trflx_o[(mkx+1)*ncnst];

double *sstr0;//                              !  Linear slope of environmental tracers [ #/Pa, kg/kg/Pa ]


double *trten;//                              !  Tendency of tracers [ #/s, kg/kg/s ]
double *trflx;//                          !  Flux of tracers due to convection [ # * kg/m2/s, kg/kg * kg/m2/s ]

double *tru;//                              !  Updraft tracers [ #, kg/kg ]

double *tru_emf;//                          !  Penetrative Downdraft tracers at entraining interfaces [ #, kg/kg ] 

double *tr0;//[mkx*ncnst];//                                !  Environmental tracers [ #, kg/kg ]

hipMalloc((void**) &trten_s,mix*mkx*ncnst*sizeof(double));
hipMalloc((void**) &tr0_o,mix*mkx*ncnst*sizeof(double));
hipMalloc((void**) &sstr0_o,mix*mkx*ncnst*sizeof(double));
hipMalloc((void**) &sstr0,mix*mkx*ncnst*sizeof(double));
hipMalloc((void**) &trten,mix*mkx*ncnst*sizeof(double));
hipMalloc((void**) &trflx,mix*(mkx+1)*ncnst*sizeof(double));
hipMalloc((void**) &tru,mix*(mkx+1)*ncnst*sizeof(double));
hipMalloc((void**) &tru_emf,mix*(mkx+1)*ncnst*sizeof(double));
hipMalloc((void**) &tr0,mix*mkx*ncnst*sizeof(double));

double *qv0_s  , *ql0_s   , *qi0_s   , *s0_s    , *u0_s    ,      
           		  *v0_s   , *t0_s    , *qt0_s    , *qvten_s ,
           		  *qlten_s, *qiten_s , *qrten_s , *qsten_s , *sten_s  , *evapc_s , 
           		  *uten_s , *vten_s  , *cufrc_s , *qcu_s   , *qlu_s   , *qiu_s   , 
           		  *qc_s;  
hipMalloc((void**) &qv0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &ql0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qi0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &s0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &u0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &v0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &t0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qt0_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qvten_s,mix*mkx*sizeof(double));

hipMalloc((void**) &qlten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qiten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qrten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qsten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &sten_s,mix*mkx*sizeof(double));

hipMalloc((void**) &evapc_s,mix*mkx*sizeof(double));
hipMalloc((void**) &uten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &vten_s,mix*mkx*sizeof(double));
hipMalloc((void**) &cufrc_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qcu_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qlu_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qiu_s,mix*mkx*sizeof(double));
hipMalloc((void**) &qc_s,mix*mkx*sizeof(double));

double *qv0_o, *ql0_o, *qi0_o, *t0_o, *s0_o, *u0_o, *v0_o;
double *qt0_o   , *thl0_o   , *thvl0_o   ,                                               
				  *thv0bot_o, *thv0top_o, *thvl0bot_o, *thvl0top_o,             
				  *ssthl0_o, *ssqt0_o  , *ssu0_o    , *ssv0_o;   

hipMalloc((void**) &qv0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &ql0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &qi0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &t0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &s0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &u0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &v0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &qt0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thl0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thvl0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thv0bot_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thv0top_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thvl0bot_o,mix*mkx*sizeof(double));
hipMalloc((void**) &thvl0top_o,mix*mkx*sizeof(double));
hipMalloc((void**) &ssthl0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &ssqt0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &ssu0_o,mix*mkx*sizeof(double));
hipMalloc((void**) &ssv0_o,mix*mkx*sizeof(double));

 double  *exit_conden;
 double  *umf_s, *slflx_s, *qtflx_s;
 double    *qlten_sink,    *qiten_sink,    *nlten_sink,    *niten_sink,    *evapc;
 double    *slflx,    *qtflx,    *uflx,    *vflx,    *cufrc,    *trflx_d;
 double    *trflx_u,    *thlu_emf,    *qtu_emf,    *uu_emf,    *vu_emf;
 double    *uemf,    *comsub,    *qc_l;
 double    *qc_i,    *qrten,    *qsten,    *qv0_star,    *ql0_star,    *qi0_star;
 double    *s0_star,    *cldfrct,    *concldfrct,    *flxrain,    *flxsnow;
 double    *ntraprd,    *ntsnprd,    *thv0bot,    *thv0top,    *thvl0bot,    *thvl0top;
 double    *dwten,    *diten,    *thvu,    *rei,    *qvten,    *qlten;
 double    *qiten,    *sten,    *uten,    *vten;                                    

 hipMalloc((void**) &exit_conden,mix*sizeof(double));
 hipMalloc((void**) &umf_s,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &slflx_s,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &qtflx_s,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &qlten_sink,mix*mkx*sizeof(double));
 hipMalloc((void**) &qiten_sink,mix*mkx*sizeof(double));
 hipMalloc((void**) &nlten_sink,mix*mkx*sizeof(double));
 hipMalloc((void**) &niten_sink,mix*mkx*sizeof(double));
 hipMalloc((void**) &evapc,mix*mkx*sizeof(double));
 hipMalloc((void**) &slflx,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &qtflx,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &uflx,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &vflx,mix*(mkx+1)*sizeof(double));

 hipMalloc((void**) &cufrc,mix*mkx*sizeof(double));
 hipMalloc((void**) &trflx_d,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &trflx_u,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &thlu_emf,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &qtu_emf,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &uu_emf,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &vu_emf,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &uemf,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &comsub,mix*mkx*sizeof(double));
 hipMalloc((void**) &qc_l,mix*mkx*sizeof(double));

 hipMalloc((void**) &qc_i,mix*mkx*sizeof(double));
 hipMalloc((void**) &qrten,mix*mkx*sizeof(double));
 hipMalloc((void**) &qsten,mix*mkx*sizeof(double));
 hipMalloc((void**) &qv0_star,mix*mkx*sizeof(double));
 hipMalloc((void**) &ql0_star,mix*mkx*sizeof(double));
 hipMalloc((void**) &qi0_star,mix*mkx*sizeof(double));

 hipMalloc((void**) &s0_star,mix*mkx*sizeof(double));
 hipMalloc((void**) &cldfrct,mix*mkx*sizeof(double));
 hipMalloc((void**) &concldfrct,mix*mkx*sizeof(double));
 hipMalloc((void**) &flxrain,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &flxsnow,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &ntraprd,mix*mkx*sizeof(double));
 hipMalloc((void**) &ntsnprd,mix*mkx*sizeof(double));
 hipMalloc((void**) &thv0bot,mix*mkx*sizeof(double));
 hipMalloc((void**) &thv0top,mix*mkx*sizeof(double));
 hipMalloc((void**) &thvl0bot,mix*mkx*sizeof(double));
 hipMalloc((void**) &thvl0top,mix*mkx*sizeof(double));
 hipMalloc((void**) &dwten,mix*mkx*sizeof(double));
 hipMalloc((void**) &diten,mix*mkx*sizeof(double));
 hipMalloc((void**) &thvu,mix*(mkx+1)*sizeof(double));
 hipMalloc((void**) &rei,mix*mkx*sizeof(double));
 hipMalloc((void**) &qvten,mix*mkx*sizeof(double));
 hipMalloc((void**) &qlten,mix*mkx*sizeof(double));
 hipMalloc((void**) &qiten,mix*mkx*sizeof(double));
 hipMalloc((void**) &sten,mix*mkx*sizeof(double));
 hipMalloc((void**) &uten,mix*mkx*sizeof(double));
 hipMalloc((void**) &vten,mix*mkx*sizeof(double));
...
//////////////////////////////////////////////////////////////////////
		double constituents_mp_qmin_[ncnst];
		...
		double wv_saturation_mp_estbl_[250];
...
//omp_set_nested(2);
#pragma omp parallel num_thread(2) // default(shared) num_thread(2)
{
...
#pragma omp master
{
for(int tempi=myid;tempi<(15120000*10/11);tempi+=size)
{
...
//ret5 = GPTLstart("cudaMemcpy1");
	hipMemcpy(p0_d, p0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(u0_d, u0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(v0_d, v0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(z0_d, z0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(dp0_d, dp0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(dpdry0_d, dpdry0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(qv0_d, qv0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(ql0_d, ql0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(qi0_d, qi0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(t0_d, t0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(s0_d, s0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(cldfrct_d, cldfrct_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(concldfrct_d, concldfrct_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);

	hipMemcpy(tr0_d, tr0_c, pcnst*mix*mkx*sizeof(double),hipMemcpyHostToDevice);

	hipMemcpy(ps0_d, ps0_c, mix*(mkx+1)*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(zs0_d, zs0_c, mix*(mkx+1)*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(tke_d, tke_c, mix*(mkx+1)*sizeof(double),hipMemcpyHostToDevice);

	hipMemcpy(pblh_d, pblh_c, mix*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(cush_d, cush_c, mix*sizeof(double),hipMemcpyHostToDevice);
...
//	hipMemcpy(tw0_in, tw0_c, pcols*pver*sizeof(double),hipMemcpyHostToDevice);
	hipMemcpy(qw0_in, qw0_c, mix*mkx*sizeof(double),hipMemcpyHostToDevice);
hipDeviceSynchronize();
...
//ret6 = GPTLstart("cudaMemcpy2");
	//	hipMemcpy
	//hipMemcpy(pblh, pblh_d, mix*sizeof(double),hipMemcpyDeviceToHost);
hipEvent_t nstart33,nstop33;
hipEventCreate(&nstart33);
hipEventCreate(&nstop33);
hipEventRecord(nstart33);
	hipMemcpy(cush_c, cush_d, mix*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(snow_c, snow_d, mix*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(precip_c, precip_d, mix*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(cbmf_c, cbmf_d, mix*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(rliq_c, rliq_d, mix*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(cnt_c, cnt_d, mix*sizeof(double),hipMemcpyDeviceToHost);

	hipMemcpy(umf_c, umf_d, mix*(mkx+1)*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(slflx_c, slflx_d, mix*(mkx+1)*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qtflx_c, qtflx_d, mix*(mkx+1)*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(flxprc1_c, flxprc1_d, mix*(mkx+1)*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(flxsnow1_c, flxsnow1_d, mix*(mkx+1)*sizeof(double),hipMemcpyDeviceToHost);

	hipMemcpy(qrten_c, qrten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qvten_c, qvten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qlten_c, qlten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qiten_c, qiten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(sten_c, sten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(uten_c, uten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(vten_c, vten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qsten_c, qsten_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(evapc_c, evapc_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(cufrc_c, cufrc_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qcu_c, qcu_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qlu_c, qlu_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qiu_c, qiu_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(qc_c, qc_d, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
	hipMemcpy(cnb_c, cnb_d, mix*sizeof(double),hipMemcpyDeviceToHost);

	hipMemcpy(trten_c, trten_d, ncnst*mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
hipDeviceSynchronize();
...
for(int tempi=myid;tempi<(15120000/11);tempi+=size)
{
//	printf("id=%d\n",id);
...
//ret6 = GPTLstop("cudaMemcpy2");	
	//hipMemcpy(qw0_c, qw0_in, mix*mkx*sizeof(double),hipMemcpyDeviceToHost);
