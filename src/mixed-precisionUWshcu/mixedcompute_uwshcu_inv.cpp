...
__device__ void positive_moisture_single(int i,float xlv,float xls,int mkxtemp,float dt,float qvmin,float qlmin,float qimin,float *dp,float *qv, 
							  			 float *ql,float *qi,float *s,float *qvten,float *qlten,float *qiten,float *sten );
...
__global__ void compute_uwshcu(
	...
	float *dwten,   float *diten,   float *thvu,   float *rei,   float *qvten,   float *qlten,
	float *qiten,   float *sten,   float *uten,   float *vten,
	...
	float *umf_out  ,float *slflx_out  ,float *qtflx_out     ,                  
	float *flxprc1_out  ,float *flxsnow1_out  ,		            
	float *qvten_out,float *qlten_out  ,float *qiten_out     ,                 
	float *sten_out ,float *uten_out   ,float *vten_out      ,float *trten_out ,        
...
//////////////////////////////////////////////////////////
	...
	////////////////////////////////////////////////
//	half2 bbb = __float2half2_rn(0.0f);
//	half2 aaa = __float2half2_rn(0.0f);
//	float ccc = 0.0;
//	half2 ddd = bbb + aaa +  __float2half2_rn(ccc);
    //1. Input variables
 ...
 //   float    sstr0[mkx*ncnst];//                              !  Linear slope of environmental tracers [ #/Pa, kg/kg/Pa ]
   ...
   //! 3. Variables associated with cumulus convection
    ...
    float    umf[mkx+1];//                                    !  Updraft mass flux at the interfaces [ kg/m2/s ]
...
void compute_uwshcu_inv_(int myid ,int size, int pcols_c ,int pver_c  ,int ncol_c  ,int pcnst_c   ,float ztodt_c  ,       
	float *ps0_inv  ,float *zs0_inv    ,float *p0_inv        , float *z0_inv    ,float *dp0_inv  ,  
	float *u0_inv   ,float *v0_inv     ,float *qv0_inv       ,float *ql0_inv   ,float *qi0_inv  ,  
	float *t0_inv   ,float *s0_inv     ,float *tr0_inv       ,                         
	float *tke_inv  ,float *cldfrct_inv,float *concldfrct_inv,float *pblh      ,float *cush     ,   
	float *umf_inv  ,float *slflx_inv  ,float *qtflx_inv     ,                          
	float *flxprc1_inv,float *flxsnow1_inv,     				 
	float *qvten_inv,float *qlten_inv  ,float *qiten_inv     ,                         
	float *sten_inv ,float *uten_inv   ,float *vten_inv      ,float *trten_inv ,               
	float *qrten_inv,float *qsten_inv  ,float *precip        ,float *snow      ,float *evapc_inv,  
	float *cufrc_inv,float *qcu_inv    ,float *qlu_inv       ,float *qiu_inv   ,                
	float *cbmf     ,float *qc_inv     ,float *rliq          ,                         
	float *cnt_inv  ,float *cnb_inv    ,int lchnk_c     ,float *dpdry0_inv )
{
	int pcols=pcols_c;
	int pver=pver_c;
	int ncol=ncol_c;
...
//ret2 = GPTLstart("Malloc");
	...
	// //////////////////////output/////////////////
	float *umf_c;
	hipHostMalloc((float**) &umf_c,mix*(mkx+1)*sizeof(float));
	float *qvten_c;
	hipHostMalloc((float**) &qvten_c,mix*mkx*sizeof(float));
	float *qlten_c;
	hipHostMalloc((float**) &qlten_c,mix*mkx*sizeof(float));
	float *qiten_c;
	hipHostMalloc((float**) &qiten_c,mix*mkx*sizeof(float));
	float *sten_c;
	hipHostMalloc((float**) &sten_c,mix*mkx*sizeof(float));
	float *uten_c;
...
hipMalloc((void**) &umf_d,mix*(mkx+1)*sizeof(float));

hipMalloc((void**) &qvten_d,mix*mkx*sizeof(float));

hipMalloc((void**) &qlten_d,mix*mkx*sizeof(float));

hipMalloc((void**) &qiten_d,mix*mkx*sizeof(float));

hipMalloc((void**) &sten_d,mix*mkx*sizeof(float));
...
hipMalloc((void**) &ssv0_o,mix*mkx*sizeof(float));
 ...
 float    *dwten,    *diten,    *thvu,    *rei,    *qvten,    *qlten;
 float    *qiten,    *sten,    *uten,    *vten;                                    
...
//ret5 = GPTLstart("cudaMemcpy1");
	...
	hipMemcpy(pblh_d, pblh, mix*sizeof(float),hipMemcpyHostToDevice);
	hipMemcpy(cush_d, cush, mix*sizeof(float),hipMemcpyHostToDevice);
...
hipEventRecord(nstart11);
		compute_uwshcu<<<dim3(ceil(ncol/128.0)),dim3(128)>>>(
...
hipEventRecord(nstart33);
	hipMemcpy(cush, cush_d, mix*sizeof(float),hipMemcpyDeviceToHost);
	hipMemcpy(snow, snow_d, mix*sizeof(float),hipMemcpyDeviceToHost);
	hipMemcpy(precip, precip_d, mix*sizeof(float),hipMemcpyDeviceToHost);
	hipMemcpy(cbmf, cbmf_d, mix*sizeof(float),hipMemcpyDeviceToHost);
	hipMemcpy(rliq, rliq_d, mix*sizeof(float),hipMemcpyDeviceToHost);
	hipMemcpy(cnt_c, cnt_d, mix*sizeof(float),hipMemcpyDeviceToHost);

	hipMemcpy(umf_c, umf_d, mix*(mkx+1)*sizeof(float),hipMemcpyDeviceToHost);
	...
	hipMemcpy(qvten_c, qvten_d, mix*mkx*sizeof(float),hipMemcpyDeviceToHost);
	...
	hipMemcpy(sten_c, sten_d, mix*mkx*sizeof(float),hipMemcpyDeviceToHost);
...
int main(int argc, char *argv[])
{
	...
	float *ps0_inv,*zs0_inv,*p0_inv,*z0_inv;
	float *dp0_inv,*u0_inv,*v0_inv,*qv0_inv,*ql0_inv,*qi0_inv;
	float *t0_inv,*s0_inv,*tr0_inv,*tke_inv;
	float *cldfrct_inv,*concldfrct_inv,*pblh,*cush,*dpdry0_inv;

	ps0_inv = (float*)calloc(pvers*pcols,sizeof(float));
	zs0_inv = (float*)calloc(pvers*pcols,sizeof(float));
	p0_inv = (float*)calloc(pver*pcols,sizeof(float));
	z0_inv = (float*)calloc(pver*pcols,sizeof(float));
	dp0_inv = (float*)calloc(pver*pcols,sizeof(float));
	u0_inv = (float*)calloc(pver*pcols,sizeof(float));
	v0_inv = (float*)calloc(pver*pcols,sizeof(float));
	qv0_inv = (float*)calloc(pver*pcols,sizeof(float));
	ql0_inv = (float*)calloc(pver*pcols,sizeof(float));
	qi0_inv = (float*)calloc(pver*pcols,sizeof(float));
	t0_inv = (float*)calloc(pver*pcols,sizeof(float));
	s0_inv = (float*)calloc(pver*pcols,sizeof(float));
	tr0_inv = (float*)calloc(pcnst*pver*pcols,sizeof(float));
	tke_inv = (float*)calloc(pvers*pcols,sizeof(float));
	cldfrct_inv = (float*)calloc(pver*pcols,sizeof(float));
	concldfrct_inv = (float*)calloc(pver*pcols,sizeof(float));
	pblh = (float*)calloc(pcols,sizeof(float));
	cush = (float*)calloc(pcols,sizeof(float));
	dpdry0_inv = (float*)calloc(pver*pcols,sizeof(float));
	...
	float *umf_inv,*slflx_inv,*qtflx_inv,*flxprc1_inv,*flxsnow1_inv;
	float *qvten_inv,*qlten_inv,*qiten_inv,*sten_inv;
	float *uten_inv,*vten_inv,*trten_inv;
	float *qrten_inv,*qsten_inv,*precip,*snow,*evapc_inv;
	float *cufrc_inv,*qcu_inv,*qlu_inv,*qiu_inv;
	float *cbmf,*qc_inv,*rliq,*cnt_inv,*cnb_inv;

	umf_inv = (float*)calloc(pvers*pcols,sizeof(float));
	slflx_inv = (float*)calloc(pvers*pcols,sizeof(float));
	qtflx_inv = (float*)calloc(pvers*pcols,sizeof(float));
	flxprc1_inv = (float*)calloc(pvers*pcols,sizeof(float));
	flxsnow1_inv = (float*)calloc(pvers*pcols,sizeof(float));
	qvten_inv = (float*)calloc(pver*pcols,sizeof(float));
	qlten_inv = (float*)calloc(pver*pcols,sizeof(float));
	qiten_inv = (float*)calloc(pver*pcols,sizeof(float));
	sten_inv = (float*)calloc(pver*pcols,sizeof(float));
	uten_inv = (float*)calloc(pver*pcols,sizeof(float));
	vten_inv = (float*)calloc(pver*pcols,sizeof(float));
	trten_inv = (float*)calloc(pcnst*pver*pcols,sizeof(float));
	qrten_inv = (float*)calloc(pver*pcols,sizeof(float));
	qsten_inv = (float*)calloc(pver*pcols,sizeof(float));
	precip = (float*)calloc(pcols,sizeof(float));
	snow = (float*)calloc(pcols,sizeof(float));
	evapc_inv = (float*)calloc(pver*pcols,sizeof(float));
	cufrc_inv = (float*)calloc(pver*pcols,sizeof(float));
	qcu_inv = (float*)calloc(pver*pcols,sizeof(float));
	qlu_inv = (float*)calloc(pver*pcols,sizeof(float));
	qiu_inv = (float*)calloc(pver*pcols,sizeof(float));
	cbmf = (float*)calloc(pcols,sizeof(float));
	qc_inv = (float*)calloc(pver*pcols,sizeof(float));
	rliq = (float*)calloc(pcols,sizeof(float));
	cnt_inv = (float*)calloc(pcols,sizeof(float));
	cnb_inv = (float*)calloc(pcols,sizeof(float));
	if (cnb_inv==NULL) 
	{
		printf("error!\n");
		exit (1);
